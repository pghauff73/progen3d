...
static std::vector<std::string> build_logical_rules(const std::vector<std::string> &source_lines) {
	std::vector<std::string> logical_rules;
	std::string current_rule;

	for(const std::string &raw_line : source_lines){
		...
		if(starts_new_rule(trimmed)){
			logical_rules.push_back(normalize_grammar_spacing(current_rule));
			current_rule = trimmed;
		}
		else{
			current_rule += " ";
...
static void parse_rule_lines_into_grammar(Grammar *grammar, const std::vector<std::string> &logical_rules) {
	for(const std::string &logical_line : logical_rules){
		if(logical_line.empty())continue;
		std::vector<std::string> rule_sections = breakup(logical_line, "->");
		if(rule_sections.size() < 2)continue;

		std::istringstream lin(rule_sections[0] + " ->");
		std::string rulename;
		lin >> rulename;
		Rule *rule = new Rule(rulename, 1);
		std::string line = grammar->ruleBody(rule, lin, rule_sections[1]);
		if(rule_sections.size() > 2)grammar->ruleAlternate(rule, rule_sections[2]);
	}
}
